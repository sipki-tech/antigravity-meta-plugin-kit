...
// contracts from the in-CLI docs (2026-07, CLI 1.0.10) plus the traps observed
...
// agents/*.toml is validator-known but officially undocumented; check the
// observed invariants with line heuristics (a spec-compliant TOML parser is
// not worth the zero-dep budget for 5 flat fields).
function lintAgents(dir, { ok }) {
  ...
  ok(
    "agents/*.toml minimally valid",
    bad.length === 0,
    bad.join("; ") || `${tomls.length} agents (TOML-lite heuristics, not a parser)`,
  );
}
