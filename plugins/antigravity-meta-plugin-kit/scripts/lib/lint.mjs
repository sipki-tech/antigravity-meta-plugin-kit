...
// contracts from the in-CLI docs (2026-07, CLI 1.1.1) plus the traps observed
...
// Five documented events + SessionStart: refuted rumor on 1.0.16, it fires
// on CLI 1.1.1 — confirmed by a live probe with the wire contract recorded
// (2026-07-12), still absent from the builtin docs. See docs/internals.md.
...
// Extracts the plugin-relative script path from a hook command line: the
// official hooks.json-relative form (./scripts/x.mjs; cwd = the hooks.json
// dir) or the legacy ${PLUGIN_ROOT} form. Returns null when the command
// references neither (nothing we can verify then).
export function scriptPathOf(command) {
  const s = String(command ?? "");
  const legacy = /\$\{PLUGIN_ROOT\}\/([^"'\s]+)/.exec(s);
  if (legacy) return legacy[1];
  const rel = /(?:^|[\s"'])\.\/([^"'\s]+)/.exec(s);
  return rel ? rel[1] : null;
}

// ${PLUGIN_ROOT} expands to an empty string on CLI 1.1.1 — every hook using
// it dies with "Cannot find module '/scripts/...'" (observed 2026-07-12).
export function usesPluginRoot(command) {
  return String(command ?? "").includes("${PLUGIN_ROOT}");
}
...
function lintHooks(dir, manifest, { ok, warn }) {
  ...
  for (const ue of unknownEvents) {
    warn("unknown hook event", `${ue} — known events: ${KNOWN_EVENTS.join(", ")}`);
  }
  ...
  const notFailOpen = [];
  for (const l of leaves) {
    if (usesPluginRoot(l.hook?.command)) {
      warn(
        "hook command uses ${PLUGIN_ROOT}",
        `${l.hookName}.${l.event}: expands to an empty string on CLI 1.1.1 ` +
          "(observed 2026-07-12) — use the hooks.json-relative form " +
          "(node ./scripts/x.mjs; cwd = the hooks.json dir)",
      );
    }
    const rel = scriptPathOf(l.hook?.command);
    if (!rel) continue; // command references no plugin-local script; nothing to verify
  ...
  ok(
    ...
    missingScripts.join(", ") || "all plugin-local script refs resolve",
  );
  ok(
