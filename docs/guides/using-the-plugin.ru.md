# Повседневная работа с установленным meta-kit плагином

*[English](using-the-plugin.md) | Русский*

Что вы реально получаете после
`npx github:sipki-tech/antigravity-meta-plugin-kit install` — и как этим
управлять из живой сессии Antigravity.

## После установки

1. **Перезапустите Antigravity** — загрузчик подхватывает плагины на старте.
2. Глобальная установка обслуживает все воркспейсы; slash-командам в каждом
   проекте нужен один дополнительный шаг:
   `npx github:sipki-tech/antigravity-meta-plugin-kit workflows`
   (кладёт алиасы `/meta-*` в `<project>/.agents/workflows/`).
3. Health-чек в любой момент:
   `npx github:sipki-tech/antigravity-meta-plugin-kit verify`.

## Пять скиллов — срабатывают по формулировке

Скиллы маршрутизируются по описаниям; произнесите триггер (или спросите в
его терминах):

| Вы говорите | Срабатывает | Что получаете |
|---|---|---|
| «meta-scaffold — начни новый плагин» | `meta-scaffold` | проводка по скаффолду: create → TODO → lint |
| «meta-hooks — спроектируй гард на rm -rf» | `meta-hooks` | выбор события + официальные контракты + закон fail-open |
| «meta-skills — поревьюй мой SKILL.md» | `meta-skills` | дисциплина frontmatter/триггеров/структуры |
| «meta-test — покрой мой хук тестами» | `meta-test` | zero-dep доктрина node --test |
| «meta-ship — готовим релиз» | `meta-ship` | npx-дистрибуция, гейты, чеклист версий |

## Четыре сабагента — делегируйте тяжёлое

Попросите главного агента делегировать — или используйте детерминированные
команды `/meta-*` (агенты сами slash-командами не становятся):

| Команда / просьба | Сабагент | Когда |
|---|---|---|
| `/meta-audit <payload>` | `meta-payload-auditor` | перед релизом: семантический аудит за пределами `lint` |
| `/meta-hook <намерение>` | `meta-hook-smith` | нужен спроектированный хук с тестами |
| `/meta-scout` | `meta-trap-scout` | Antigravity обновился или плагин чудит при зелёном линте |
| `/meta-mirror` | `meta-doc-mirror` | изменились EN-доки, RU-зеркало должно догнать |

Сабагенты стартуют со свежим контекстом: давайте команде всё необходимое
(пути, намерение) — вашу историю разговора они не видят.

## Типичная авторская сессия

```text
вы:  meta-scaffold — нужен плагин, блокирующий force-push
     → скаффолд создан, TODO перечислены
вы:  /meta-hook блокируй `git push --force` на PreToolUse
     → блок hooks.json + fail-open скрипт + тесты; ревью и применение
вы:  meta-test — чего не хватает?
     → добавлен e2e с мусорным stdin
вы:  /meta-audit my-plugin
     → находки BLOCKER/WARN/NIT; чините, перелинтовываете
вы:  meta-ship — чеклист релиза
     → lint + agy validate + CI-гейты → тег
```

## Troubleshooting

- **Скиллы не срабатывают после установки** — перезапустили Antigravity?
  Затем `verify`; классический тихий убийца — отсутствующий
  `installed_version.json` (verify его проверяет).
- **`/meta-*` неизвестна в проекте** — выполните команду `workflows` в этом
  проекте; одна глобальная установка алиасы в проекты не кладёт.
- **Сабагент не спавнится** — spawnability агентов из плагинов в рантайме —
  уверенность MEDIUM (см. [internals](../internals.md)); тело workflow всё
  равно ведёт главного агента сделать работу самому — о наблюдениях
  сообщите через `/meta-scout`.
- **Старая версия** — `npx github:sipki-tech/antigravity-meta-plugin-kit#main update`
  (не забудьте `#main`: npx кэширует).

*См. также: [Быстрый старт](getting-started.ru.md) ·
[Сабагенты](agents.ru.md) · [реестр ловушек](../internals.md)*
